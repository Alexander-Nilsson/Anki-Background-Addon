...
def inject_css(web_content, context):
    for filename in web_content.css.copy():
        ...
        if filename in css_files_to_modify:
            ...
            addon_css_path = os.path.join(addon_path, "web", "css", version_folder, filename)
            if os.path.exists(addon_css_path):
                web_content.css.append(f"/_addons/{addonfoldername}/web/css/{version_folder}/{filename}")
            
            user_css_path = os.path.join(addon_path, "user_files", "css", f"custom_{filename}")
...
def get_gearfile():
    ...
    gear_list = [os.path.basename(f) for f in os.listdir(gear_abs) if f.lower().endswith(IMAGE_EXTS)]
